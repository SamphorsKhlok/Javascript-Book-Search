...
describe('FilterPipe', () => {
  ...
  it('sort by page number ', () => {
    const pipe = new FilterPipe();
    const result: Book[] = pipe.transform(books, [0, 'pages']);
    expect(result[0].pages).toBe(166);
  });
  it('sort by release Date ', () => {
    const pipe = new FilterPipe();
    const result: Book[] = pipe.transform(books, [0, 'releaseDate']);
    expect(result[0].releaseDate).toBe('12/2008');
  });
  it('sort by author ', () => {
    const pipe = new FilterPipe();
    const result: Book[] = pipe.transform(books, [0, 'author']);
    expect(result[0].author).toBe('Douglas Crockford');
  });
  it('200 pages filter result should be 4 ', () => {
    const pipe = new FilterPipe();
    const result = pipe.transform(books, [200, 'pages']);
    expect(result.length).toBe(4);
  });
  it('500 pages filter result should be 1 ', () => {
    const pipe = new FilterPipe();
    const result = pipe.transform(books, [500, 'pages']);
